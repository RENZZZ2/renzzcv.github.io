    ...
    for (const file of files) {
        ...
        reader.onload = function(event) {
            ...
            lines.forEach((line, index) => {
                ...
                if ((index + 1) % contactCount === 0 || index === lines.length - 1) {
...
function convertVcfToTxt() {
    ...
    const processFile = (fileContent) => {
        // Pisahkan setiap vCard dan ekstrak nomor telepon
        const vcardArray = fileContent.split('END:VCARD').map(vcard => vcard.trim() + 'END:VCARD').filter(vcard => vcard.trim() !== '');
