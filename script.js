...
function convertTxtToVcf() {
    ...
    if (!contactCount || !contactName || !vcfFileName) {
        alert('Silakan masukkan jumlah kontak, nama kontak, dan nama file VCF.');
        return;
    }
    ...
    reader.onload = function(event) {
        ...
        while (index < lines.length) {
            let vcfFileContent = `BEGIN:VCARD\nVERSION:3.0\nFN:${contactName} ${Math.floor(index / contactCount) + 1}\n`;
            ...
            link.href = vcfUrl;
            link.download = `${vcfFileName}_${Math.floor(index / contactCount)}.vcf`;
            link.click();
